"""
PFES + SAMOTA Integration - EI VARIANT
Single change from baseline PFES_SAMOTA.py:
  GS candidate selection uses Expected Improvement (EI) instead of separate best+uncertain.

CHANGE:
  Old: top-1 by lowest predicted score  +  top-1 by highest uncertainty  (2 heuristics, independent)
  New: top-2 by Expected Improvement    (one principled formula combining both)

WHY EI IS BETTER:
  EI(x) = (f_best - mu - xi) * Phi(Z) + sigma * phi(Z)   where Z = (f_best - mu - xi) / sigma
  Reference: Jones, Schonlau & Welch (1998), "Efficient Global Optimization of Expensive
             Black-Box Functions", Journal of Global Optimization. doi:10.1023/A:1008306431147

  When objective has NO violations yet (f_best > 0):
    EI pushes toward UNCERTAIN regions near the best score seen -- natural exploration with direction
  When objective HAS violations (f_best < 0):
    EI balances exploitation (find more violations) vs exploration (uncertain regions)
    The balance shifts automatically as more data is collected

  The old heuristic always splits 50/50 between best and uncertain regardless of
  what we already know. EI adjusts the balance data-adaptively.

KEY FEATURES (same as PFES_SAMOTA.py):
  Phase 1 (ART): Maximin sampling for diverse population
  Phase 2a (GS): PER-OBJECTIVE surrogates + EI acquisition function  <-- CHANGED
  Phase 2b (LS): NSGA3 with SINGLE RBF per cluster per objective      (unchanged)

Offline Phase: Parametric model checking → constraints
Online Phase: SAMOTA (Phase 1: ART + Phase 2: GS[EI]+LS with surrogates)
"""
...
def pfes_samota_ei(max_iterations=1000, max_time_seconds=float("inf"), budget=900):
    """
    ...
    import logging
    log_file = "pfes_samota_ei/samota_detailed.log"
    os.makedirs("pfes_samota_ei", exist_ok=True)
    ...
    if not os.path.exists("pfes_samota_ei"):
        os.makedirs("pfes_samota_ei", exist_ok=True)
    ...
    best_scores_df.to_csv('pfes_samota_ei/score_NSGA3_1.csv', index=False)
    print("\n✓ Saved: pfes_samota_ei/score_NSGA3_1.csv")
    ...
    reqs_df.to_csv('pfes_samota_ei/reqs_NSGA3_1.csv', index=False)
    print("✓ Saved: pfes_samota_ei/reqs_NSGA3_1.csv")
    ...
    X_df.to_csv('pfes_samota_ei/X_all_evaluations_NSGA3_0.csv', index=False)
    print("✓ Saved: pfes_samota_ei/X_all_evaluations_NSGA3_0.csv")

    F_df = pd.DataFrame(
        database_processed,
        columns=[f'V{i}' for i in range(len(database_processed[0]) if database_processed else len(conf.CONSTRAINTS))]
    )
    F_df.to_csv('pfes_samota_ei/F_all_evaluations_NSGA3_0.csv', index=False)
    print("✓ Saved: pfes_samota_ei/F_all_evaluations_NSGA3_0.csv")
...
if __name__ == "__main__":
    results = pfes_samota_ei(max_iterations=1000, budget=900)
