"""
PFES + SAMOTA Integration - EI + NBI VARIANT
Builds on PFES_SAMOTA_EI.py with one additional change:
  Near-Boundary Intensification (NBI) after each LS phase.

CHANGES vs PFES_SAMOTA.py:
  1. GS: Expected Improvement acquisition function (from PFES_SAMOTA_EI.py)
  2. NBI: After each LS, check if any uncovered objective has score in (0, threshold).
          If so, generate small Gaussian perturbations around the best test case found
          for that objective and evaluate them immediately.

WHY NBI:
  Experiments consistently show V2 ending at ~0.003 -- tantalizingly close to 0
  (violation boundary) but never crossing it. GS and LS both operate at the
  population level; neither has a mechanism to "zoom in" on a near-miss.

  NBI adds a precision step: once the algorithm is within threshold of a violation,
  it perturbs that specific test case with small noise (5% of parameter range) to
  find the exact crossing point.

  Reference: intensification concept from:
    Feo & Resende (1995), "Greedy Randomized Adaptive Search Procedures",
    J. Global Optimization 6(2):109-133. doi:10.1007/BF01096763
    Conn, Gould & Toint (2000), "Trust Region Methods", SIAM.
    -- Both show that alternating intensification (exploit near-miss) with
       diversification (GS/LS broad search) outperforms either alone.

NBI PARAMETERS:
  NEAR_BOUNDARY_THRESHOLD = 0.05  -- trigger when score in (0, 0.05)
  NBI_N_CANDIDATES = 10           -- max perturbations evaluated per objective
  NBI_SIGMA_FRACTION = 0.05       -- perturbation = 5% of each parameter's range

KEY FEATURES:
  Phase 1 (ART): Maximin sampling for diverse population             (unchanged)
  Phase 2a (GS): PER-OBJECTIVE surrogates + EI acquisition function  (from EI variant)
  Phase 2b (LS): NSGA3 with SINGLE RBF per cluster per objective     (unchanged)
  Phase 2c (NBI): Gaussian perturbation of near-boundary test cases  <-- NEW

Offline Phase: Parametric model checking → constraints
Online Phase: SAMOTA (Phase 1: ART + Phase 2: GS[EI] + LS + NBI)
"""
...
from scipy.stats import norm as scipy_norm  # For EI acquisition function
...
def global_search_nsga3(X_array, F_array, uncovered_objectives, pop_size=30, n_gen=50, top_k=1):
    ...
    EI VARIANT: Candidate selection uses Expected Improvement acquisition function
    instead of the heuristic best+uncertain split.
    ...
    For EACH uncovered objective:
      ...
      3. From GA population, compute EI for each candidate:
           EI(x) = (f_best - mu - xi) * Phi(Z) + sigma * phi(Z)
           Z = (f_best - mu - xi) / sigma
         where:
           f_best = best (lowest) score seen for this objective in the database
           mu     = surrogate predicted score for x
           sigma  = surrogate uncertainty (ensemble disagreement) for x
           xi     = 0.01 exploration bonus
      4. Select top-2 candidates by EI (maintains same total candidate count as best+uncertain)

    EI behaviour:
      - No violations found yet (f_best > 0): EI ≈ sigma * phi(Z) -> favours uncertain regions
        near f_best. Natural exploration with directional bias toward the best seen score.
      - Violations found (f_best < 0): EI balances exploitation vs exploration automatically.
        Balance shifts toward exploitation as more violations are found.

    Reference: Jones, Schonlau & Welch (1998), "Efficient Global Optimization of Expensive
    ...
    for obj_idx in uncovered_objectives:
        ...
        # Replaces heuristic best+uncertain split with principled Expected Improvement.
        # Reference: Jones, Schonlau & Welch (1998) doi:10.1023/A:1008306431147
        #
        # f_best: best score seen so far for this objective in the real database.
        #   If no violations: f_best > 0 (e.g. 0.011). EI favours uncertain regions near f_best.
        #   If violations found: f_best < 0. EI balances exploit/explore automatically.
        f_best = float(np.min(F_array[:, obj_idx]))
        xi = 0.01  # Small exploration bonus; keeps EI > 0 even at current best

        ei_scored = []
        for params, pred, unc in scored_pop:
            sigma = max(float(unc), 1e-9)  # Numerical stability
            Z = (f_best - pred - xi) / sigma
            ei = (f_best - pred - xi) * scipy_norm.cdf(Z) + sigma * scipy_norm.pdf(Z)
            ei_scored.append((params, pred, unc, max(float(ei), 0.0)))

        # Select top-2 by EI per objective (same total count as best+uncertain)
        ei_scored.sort(key=lambda t: t[3], reverse=True)
        for params, pred, unc, ei in ei_scored[:2]:
...
def pfes_samota_ei_nbi(max_iterations=1000, max_time_seconds=float("inf"), budget=900):
    """
    PFES + SAMOTA Integration on ADAS1 -- EI VARIANT

    Single change vs pfes_samota(): GS uses Expected Improvement acquisition function
    instead of heuristic best+uncertain candidate selection.

    Offline: Parametric model checking (skipped - use constraints)
    Online:
      Phase 1: ART (Adaptive Random Testing)
      Phase 2: SAMOTA (GS[EI] + LS iterations)
    ...
    import logging
    log_file = "pfes_samota_ei_nbi/samota_detailed.log"
    os.makedirs("pfes_samota_ei_nbi", exist_ok=True)
    ...
    for iteration in range(max_iterations):
        ...
        # ====================================================================
        # Near-Boundary Intensification (NBI)
        # Triggered when any uncovered objective has score in (0, threshold).
        # Generates small Gaussian perturbations around the best test case
        # found for that objective, trying to push it over the violation boundary.
        #
        # Reference: intensification concept from Feo & Resende (1995),
        # "Greedy Randomized Adaptive Search Procedures", J. Global Optimization.
        # doi:10.1007/BF01096763
        # ====================================================================
        NBI_THRESHOLD = 0.05      # Trigger if score in (0, 0.05)
        NBI_N_CANDIDATES = 10     # Max simulator evals per near-boundary objective
        NBI_SIGMA_FRACTION = 0.05 # Perturbation = 5% of each parameter's range

        F_nbi = np.array(database_processed)
        current_best_nbi = np.min(F_nbi, axis=0)
        near_boundary_objs = [
            obj_idx for obj_idx in uncovered_objectives
            if 0 < current_best_nbi[obj_idx] < NBI_THRESHOLD
        ]

        if near_boundary_objs and eval_count < budget:
            logger.info(f"  NBI triggered for objectives: {near_boundary_objs}")
            print(f"  Near-Boundary Intensification (NBI) for V{near_boundary_objs}...")

            var_names_nbi = sorted(conf.SS_VARIABLES.keys())
            lb_nbi = np.array([conf.SS_VARIABLES[v]["range"][0] for v in var_names_nbi])
            ub_nbi = np.array([conf.SS_VARIABLES[v]["range"][1] for v in var_names_nbi])
            sigma_nbi = (ub_nbi - lb_nbi) * NBI_SIGMA_FRACTION

            nbi_violations_before = len(archive)
            nbi_start_evals = eval_count

            for obj_idx in near_boundary_objs:
                if eval_count >= budget:
                    break

                # Best test case for this objective in the real database
                best_db_idx = int(np.argmin(F_nbi[:, obj_idx]))
                best_x = np.array(database_X[best_db_idx])
                best_score = float(current_best_nbi[obj_idx])
                logger.info(f"    NBI V{obj_idx}: best_score={best_score:.6f}, zooming in...")

                nbi_evals_this_obj = 0
                attempts = 0
                while nbi_evals_this_obj < NBI_N_CANDIDATES and eval_count < budget:
                    attempts += 1
                    if attempts > NBI_N_CANDIDATES * 5:
                        break  # Give up if too many duplicates

                    # Gaussian perturbation around best point, clipped to bounds
                    perturbed = best_x + np.random.normal(0, sigma_nbi)
                    perturbed = np.clip(perturbed, lb_nbi, ub_nbi)
                    for i, v in enumerate(var_names_nbi):
                        if conf.SS_VARIABLES[v]["domain"] == int:
                            perturbed[i] = float(int(round(perturbed[i])))

                    # Skip duplicates
                    if len(database_X) > 0 and any(
                        np.allclose(perturbed, ex, atol=1e-3) for ex in database_X
                    ):
                        continue

                    # Evaluate
                    params_nbi = list(perturbed)
                    raw_est, proc_scores, reqs_sat = evaluate_test_case(params_nbi)
                    eval_count += 1
                    nbi_evals_this_obj += 1

                    tc_nbi = {
                        v: (float(perturbed[i]) if conf.SS_VARIABLES[v]["domain"] == float
                            else int(perturbed[i]))
                        for i, v in enumerate(var_names_nbi)
                    }
                    database.append(tc_nbi)
                    database_X.append(perturbed)
                    database_F.append(raw_est)
                    database_processed.append(proc_scores)

                    logger.info(
                        f"    NBI V{obj_idx} candidate {nbi_evals_this_obj}: "
                        f"score={proc_scores[obj_idx]:.6f}, "
                        f"violated={[i for i,r in enumerate(reqs_sat) if not r]}"
                    )

                    for req_idx, req_sat in enumerate(reqs_sat):
                        if not req_sat:
                            unsatisfied_reqs[req_idx] += 1
                    if any(not r for r in reqs_sat):
                        archive.append(tc_nbi)

            X_array = np.array(database_X)
            F_array = np.array(database_F)
            F_processed = np.array(database_processed)

            nbi_evals_used = eval_count - nbi_start_evals
            nbi_violations_found = len(archive) - nbi_violations_before
            print(f"    NBI: {nbi_evals_used} evals, {nbi_violations_found} new violations")
            logger.info(f"    NBI: {nbi_evals_used} evals, {nbi_violations_found} new violations")
    ...
    if not os.path.exists("pfes_samota_ei_nbi"):
        os.makedirs("pfes_samota_ei_nbi", exist_ok=True)
    ...
    best_scores_df.to_csv('pfes_samota_ei_nbi/score_NSGA3_1.csv', index=False)
    print("\n✓ Saved: pfes_samota_ei_nbi/score_NSGA3_1.csv")
    ...
    reqs_df.to_csv('pfes_samota_ei_nbi/reqs_NSGA3_1.csv', index=False)
    print("✓ Saved: pfes_samota_ei_nbi/reqs_NSGA3_1.csv")
    ...
    X_df.to_csv('pfes_samota_ei_nbi/X_all_evaluations_NSGA3_0.csv', index=False)
    print("✓ Saved: pfes_samota_ei_nbi/X_all_evaluations_NSGA3_0.csv")

    F_df = pd.DataFrame(
        database_processed,
        columns=[f'V{i}' for i in range(len(database_processed[0]) if database_processed else len(conf.CONSTRAINTS))]
    )
    F_df.to_csv('pfes_samota_ei_nbi/F_all_evaluations_NSGA3_0.csv', index=False)
    print("✓ Saved: pfes_samota_ei_nbi/F_all_evaluations_NSGA3_0.csv")
...
if __name__ == "__main__":
    results = pfes_samota_ei_nbi(max_iterations=1000, budget=900)
