...
def load_runs(pattern):
    ...
    for run_dir in sorted(glob.glob(pattern)):
        ...
        # Drop any row that cannot be converted to float (e.g. header strings)
...
def detect_reachable(algo_runs):
    """Return list of objective indices violated in >=REACHABLE_THRESHOLD of all runs."""
...
def first_violation_evals(F, reachable):
    """Dict: obj -> first eval index where violated (np.inf if never)."""
    fv = {}
